...
Function createJsonObject(vRS, kubun)
	...
	Do Until vRS.EOF
		...
		If (addFlag = True) Then
			...
			With oJSON.data(kubun)
				...
				With .item("o"&CStr(beforeOrderNo))
					.Add "d"&beforeOrderDetailNo ,oJSON.Collection()
				End With
			End With
		End If
		...
		With oJSON.data(kubun).item("o"&CStr(beforeOrderNo))
			With .item("d"&beforeOrderDetailNo)
		...
		With oJSON.data(kubun).item("o"&CStr(beforeOrderNo))
			With .item("d"&beforeOrderDetailNo)
				With .item("sub"&CStr(vRS("受注明細枝番")))
					...
					.Add "order_detail_sub_no" ,CStr(Trim(vRS("受注明細枝番")))
					.Add "kubun" ,CStr(Trim(vRS("ポイント区分")))
					.Add "point_date" ,formatDateYYYYMMDD(pointDate)
					.Add "point" ,point
					.Add "point_zan" ,point
					.Add "use_order_no" ,useOrderNo
				End With
			End With
		End With
	...
	If kubun = "obtain" Then
		oJSON.data.Add "total_obtain_point" ,totalPoint
	ElseIf kubun = "used" Then
		oJSON.data.Add "total_used_point" ,totalPoint
	End If
