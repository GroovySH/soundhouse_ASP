...
'
'更新履歴
'2009/10/06 an SeachList.aspから分離させて新規作成
...
if Err.Description <> "" then
	wErrDesc = "Outlet.asp" & " " & replace(replace(Err.Description, vbCR, " "), vbLF, " ")
...
'---- パン屑リスト作成
    call fCreateFootprintHTML("Outlet.asp","わけあり市場")  '2010/06/08 an add

'-----
if RS.EOF = true then
    wNoData = "Y"
    wNoDataHTML = wNoDataHTML & "該当する商品が見つかりません。" & vbNewLine    '2010/06/08 an add
else
...
'wSQL2 = wSQL2 & "   AND a.B品フラグ = 'Y'"
'wSQL2 = wSQL2 & "   AND b.B品引当可能数量 > 0"
'
''---- カテゴリーを指定して絞り込み
'if CategoryCd <> "" then
'    wSQL2 = wSQL2 & "  AND f.カテゴリーコード = '" & CategoryCd & "'"
'end if
'
''---- 中カテゴリーを指定して絞り込み
...
'else
'    wSQL2 = wSQL2 & "   AND (a.B品単価 * (" & wSalesTaxRate & " + 100) / 100) BETWEEN " & sPriceFrom & " AND " & sPriceTo
'end if
'
...
'Select Case i_sort
'    Case "Price_ASC"
'    	v_order = " ORDER BY a.B品単価, c.メーカー名, a.商品名"
'    Case "Price_DESC"
'    	v_order = " ORDER BY a.B品単価 DESC, c.メーカー名, a.商品名"
'    Case "MakerName_ASC"
...
'    Case Else
'    	v_order = " ORDER BY お勧め商品表示順, c.メーカー名, a.商品名"
'End Select
'
''---- 該当商品一覧SQL
...
'
''---- 絞り込み用価格帯SQL
'wSQLPricerange = "SELECT MAX(a.B品単価) AS MAX販売単価, MIN(a.B品単価) AS MIN販売単価"
...
'--- WHERE
wSQL2 = wSQL2 & "WHERE "
wSQL2 = wSQL2 & "       a.セール区分番号 = 2 "						' わけあり (Outlet) セール区分番号 : 2
...
'価格帯の検索条件を税抜き
'    wSQL2 = wSQL2 & "    AND (a.B品単価 * (" & wSalesTaxRate & " + 100) / 100) BETWEEN " & sPriceFrom & " AND " & sPriceTo & " "
    wSQL2 = wSQL2 & "    AND a.B品単価 BETWEEN " & sPriceFrom & " AND " & sPriceTo & " "
...
Select Case i_sort
    Case "Price_ASC"
        v_order = " ORDER BY a.B品単価, a.メーカー名, a.商品名 "
    Case "Price_DESC"
        v_order = " ORDER BY a.B品単価 DESC, a.メーカー名, a.商品名 "
    Case "MakerName_ASC"
...
'--- 絞り込み用価格帯SQL
wSQLPricerange = "SELECT MAX(a.B品単価) AS MAX販売単価, MIN(a.B品単価) AS MIN販売単価 " _
               & wSQL2
...
<title>わけあり市場 <%=wTitle%> 一覧｜サウンドハウス</title>
<meta name="Description" content="展示処分品や箱ナシといった棚ズレ商品を、通常の衝撃特価よりもさらにお買い得なお値段で大放出！">
<meta name="keywords" content="得割市場,B品,アウトレット,数量限定バーゲン,プライスダウン情報">
...
<!-- 絞込検索用Form START -->
            <form name="f_search" method="get" action="Outlet.asp">
