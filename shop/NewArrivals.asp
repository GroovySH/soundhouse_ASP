...
'更新履歴
'2009/10/15 an 新規作成
'2009/11/06 an 検索条件を初回入荷日ではなく発売日に変更
...
'2012/01/17 GV Webセール商品テーブルを駆動表とする対応
...
Response.AddHeader "Location", "http://www.soundhouse.co.jp/new_arrivals/index"
...
i_Nyukabi = ReplaceInput(Trim(Request("i_Nyukabi")))
...
'---- i_Nyukabiセット
if i_Nyukabi <> "" then
    i_Nyukabi = Clng(i_Nyukabi)
    if i_Nyukabi > 60 then
        i_Nyukabi = 60
    end if
else
    i_Nyukabi = 7
...
if Err.Description <> "" then
	wErrDesc = "NewArrivals.asp" & " " & replace(replace(Err.Description, vbCR, " "), vbLF, " ")
...
'---- パン屑リスト作成
call fCreateFootprintHTML("NewArrivals.asp","新商品情報")  '2010/06/08 an add
...
'wSQL = wSQL & "     , (SELECT COUNT(*) "   '2010/05/13 an changed
...
'wSQL2 = wSQL2 & "   AND a.発売日 >= DateAdd(d, " & -Clng(i_Nyukabi) & ", getdate())"    '発売日の条件
...
'    Case "ReviewCount"
'        v_order = " ORDER BY レビュー数 DESC, c.メーカー名, a.商品名"
'    Case "Zaiko_DESC"
...
'--- WHERE
wSQL2 = wSQL2 & "WHERE "
wSQL2 = wSQL2 & "        a.セール区分番号 = 5 "						' 新入荷品情報 (NewArrival) セール区分番号 : 5
wSQL2 = wSQL2 & "    AND a.発売日 >= DATEADD(d, " & CLng(i_Nyukabi) * -1 & ", GETDATE()) "
...
If RSv("メーカーロゴファイル名") <> "" Then
    wHTML = wHTML & "<img src='maker_img/" & RSv("メーカーロゴファイル名") & "' alt='" & Replace(RSv("メーカー名"),"'","&#39;") & "'>"
end if
...
<title>新商品情報 <%=wTitle%> 一覧｜サウンドハウス</title>
<meta name="Description" content="最新の新製品はもちろん、皆さんが待ちに待った到着したばかりの入荷品の情報をお知らせいたします。">
<!--#include file="../Navi/NaviStyle.inc"-->
<link rel="stylesheet" href="style/shop.css?20121108" type="text/css">
<link rel="stylesheet" href="Style/searchlist.css?20121109" type="text/css">
<link rel="stylesheet" href="style/ask.css?20140401a" type="text/css">
<script type="text/javascript">
//
//    入荷日 onChange
//
function Nyukabi_onChange(){
    for (var i=0; i<document.f_Nyukabi.Nyukabi.options.length; i++){
        if (document.f_Nyukabi.Nyukabi.options[i].selected == true){
            document.f_search.i_Nyukabi.value = document.f_Nyukabi.Nyukabi.options[i].value;
            break;
        }
    }
...
<%=wFootprintHTML%>
		...
		<img src="newarrivals/new_top2.jpg" alt="新商品情報" style="width:710px;">

<!-- ページメイン部分の記述 START -->
<!-- タブバナー部分の記述 START -->
        <div id="info_box">
            <form name="f_Nyukabi">
        	<select name="Nyukabi" onChange="Nyukabi_onChange();">
...
<!-- 絞込検索用Form START -->
      <form name="f_search" method="get" action="NewArrivals.asp">
        ...
        <input type="hidden" name="i_Nyukabi" value="<%=i_Nyukabi%>">
      </form>
<!-- 絞込検索用Form END -->
...
<%
'----NAVI用パラメータセット
'NAVILargeCategoryCd = LargeCategoryCd   '2010/06/08 an del
'NAVIMidCategoryCd = MidCategoryCd       '2010/06/08 an del
...
//    入荷日
for (var i=0; i<document.f_Nyukabi.Nyukabi.options.length; i++){
    if (document.f_Nyukabi.Nyukabi.options[i].value == document.f_search.i_Nyukabi.value){
        document.f_Nyukabi.Nyukabi.options[i].selected = true;
        break;
    }
}
