...
'
'更新履歴
'2009/10/08 an 旧Outlet.asp。SearchListCommon.incを利用するように変更し、ファイル名をSpecialPriceSale.aspに変更
...
'2012/10/22 ok 並び替えにお得順追加、絞り込み用ソート順を表示順に変更
'2013/06/26 GV #1591 ソート順変更(DESC)
...
if Err.Description <> "" then
	wErrDesc = "SpecialPriceSale.asp" & " " & replace(replace(Err.Description, vbCR, " "), vbLF, " ")
...
'---- パン屑リスト作成
call fCreateFootprintHTML("SpecialPriceSale.asp","得割市場")  '2010/06/08 an add
...
if RS.EOF = true then
	...
	wListHTML = Replace(wListHTML,"選択してください</option>","選択してください</option>" & vbNewLIne & "<option value='NewSet_DESC'>新着順</option>" & vbNewLIne & "<option value='Nesage_DESC'>お得順</option>")
...
'wSQL = wSQL & "     , CASE"
'wSQL = wSQL & "           WHEN a.特価商品表示順 = 0 THEN 99999"
'wSQL = wSQL & "           ELSE a.特価商品表示順"
'wSQL = wSQL & "       END AS 特価表示順"
'wSQL = wSQL & "     , b.色"
'wSQL = wSQL & "     , b.規格"
'wSQL = wSQL & "     , b.引当可能数量"
...
'
''---- FROM
'wSQL2 = wSQL2 & "  FROM Web商品 a WITH (NOLOCK) LEFT JOIN シリーズ g WITH (NOLOCK) ON a.シリーズコード = g.シリーズコード"
...
'wSQL2 = wSQL2 & "   AND a.取扱中止日 IS NULL"
'wSQL2 = wSQL2 & "   AND (a.廃番日 IS NULL OR (a.廃番日 IS NOT NULL AND b.引当可能数量 > 0))"
'wSQL2 = wSQL2 & "   AND (a.特価商品フラグ = 'Y' OR  g.特価シリーズフラグ = 'Y')"
...
'    Case Else
'    	v_order = " ORDER BY 特価表示順, c.メーカー名, a.商品名"
'End Select
'
''---- 該当商品一覧SQL
...
'
''---- 絞り込み用メーカーSQL
'wSQLMaker = "SELECT c.メーカー名, c.メーカーコード, d.カテゴリーコード, COUNT(DISTINCT a.商品コード) AS 商品件数"
'wSQLMaker = wSQLMaker & wSQL2    '2010/06/08 an mod
'wSQLMaker = wSQLMaker & " GROUP BY c.メーカー名, c.メーカーコード, d.カテゴリーコード"
...
'--- WHERE
wSQL2 = wSQL2 & "WHERE "
wSQL2 = wSQL2 & "       a.セール区分番号 = 1 "						' 衝撃特価 (SpecialPriceSale) セール区分番号 : 1
...
Select Case i_sort
	...
	Case "NewSet_DESC"								'2013/06/27 na
        v_order = " ORDER BY a.特価表示順 DESC, a.メーカー名, a.商品名 "
    Case Else
'2013/06/26 GV #1591 mod start
'        v_order = " ORDER BY a.特価表示順, a.メーカー名, a.商品名 "
        v_order = " ORDER BY a.特価表示順 DESC, a.メーカー名, a.商品名 "
'2013/06/26 GV #1591 mod end
End Select
...
'--- 絞り込み用メーカーSQL
wSQLMaker = "SELECT a.メーカー名, a.メーカーコード, a.カテゴリーコード, COUNT(DISTINCT a.商品コード) AS 商品件数 " _
          & wSQL2 & " " _
          & "GROUP BY a.メーカー名, a.メーカーコード, a.カテゴリーコード " _
          & "ORDER BY 1"
...
<meta name="Description" content="サウンドハウスがお薦めするとってもお買い得な商品を、衝撃特価でご提供！">
<meta name="keywords" content="得割市場,衝撃特価品,アウトレット,数量限定バーゲン,プライスダウン情報">
<title>得割市場 <%=wTitle%> 一覧｜サウンドハウス </title>
...
<!-- タブバナー部分の記述 START -->
<div class="tab" id="specialpricesale">
    <ul>
...
<!-- 絞込検索用Form START -->
          <form name="f_search" method="get" action="SpecialPriceSale.asp">
