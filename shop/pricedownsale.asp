...
'
'更新履歴
'2009/10/15 an 他のセールと表示形式を統一してリニューアル
'2009/11/05 an 前回販売価格が0円は無視するように修正
...
'2012/01/12 GV Webセール商品テーブルを駆動表とする対応
...
i_LastUpdate = ReplaceInput(Trim(Request("i_LastUpdate")))
...
'---- i_LastUpdateセット
if i_LastUpdate <> "" then
    i_LastUpdate = Clng(i_LastUpdate)
    if i_LastUpdate > 60 then
    	i_LastUpdate = 60
    end if
else
    i_LastUpdate = 7
...
if Err.Description <> "" then
	wErrDesc = "PricedownSale.asp" & " " & replace(replace(Err.Description, vbCR, " "), vbLF, " ")
...
'---- パン屑リスト作成
call fCreateFootprintHTML("PriceDownSale.asp","プライスダウン情報")  '2010/06/08 an add

if RS.EOF = true then
    wNoData = "Y"
	wNoDataHTML = wNoDataHTML & "<br />該当する商品が見つかりません。"
else
...
'wSQL = wSQL & "     , a.前回単価変更日"
'wSQL = wSQL & "     , CASE"
'wSQL = wSQL & "         WHEN a.個数限定数量 > a.個数限定受注済数量 THEN a.前回販売単価 - a.個数限定単価"
'wSQL = wSQL & "         WHEN a.B品フラグ = 'Y' THEN a.前回販売単価 - a.B品単価"
'wSQL = wSQL & "         ELSE a.前回販売単価 - a.販売単価"
'wSQL = wSQL & "       END AS 値下げ幅"
'wSQL = wSQL & "     , a.個数限定単価"
'wSQL = wSQL & "     , a.個数限定数量"
...
'wSQL = wSQL & "     , b.適正在庫数量"
'wSQL = wSQL & "     , CASE"
'wSQL = wSQL & "         WHEN b.引当可能数量 <= 0 THEN -1"                                     '2010/03/18 an changed start
'wSQL = wSQL & "         WHEN b.引当可能数量 > 0 AND b.引当可能数量 <= a.希少数量 THEN 0"
'wSQL = wSQL & "         WHEN a.個数限定数量 > 0 AND (a.個数限定数量-a.個数限定受注済数量) > 0 THEN a.個数限定数量-a.個数限定受注済数量"
'wSQL = wSQL & "         WHEN a.B品フラグ = 'Y' THEN b.B品引当可能数量"
'wSQL = wSQL & "         WHEN b.引当可能数量 > 0 AND b.引当可能数量 > a.希少数量 THEN 9999"
'wSQL = wSQL & "         ELSE 99999"
'wSQL = wSQL & "       END AS 在庫有無"                                                        '2010/03/18 an changed end
'wSQL = wSQL & "     , CASE"
...
'wSQL2 = wSQL2 & "     , 商品カテゴリー f WITH (NOLOCK)"
'
''---- WHERE
...
'wSQL2 = wSQL2 & "   AND b.色 = ''"
'wSQL2 = wSQL2 & "   AND b.規格 = ''"
'wSQL2 = wSQL2 & "   AND c.メーカーコード = a.メーカーコード"
'wSQL2 = wSQL2 & "   AND ((a.個数限定数量 > a.個数限定受注済数量 AND a.前回販売単価 > a.個数限定単価 AND a.個数限定数量 > a.個数限定受注済数量)"
'wSQL2 = wSQL2 & "   OR  (a.B品フラグ = 'Y' AND a.前回販売単価 > a.B品単価 AND B品引当可能数量 > 0)"
'wSQL2 = wSQL2 & "   OR  (a.前回販売単価 > a.販売単価  AND a.前回販売単価 != 0 AND b.引当可能数量 > 0))"
'wSQL2 = wSQL2 & "   AND a.前回単価変更日 >= DateAdd(d, " & -Clng(i_LastUpdate) & ", getdate())"     '前回価格更新日の条件
'wSQL2 = wSQL2 & "   AND d.カテゴリーコード = f.カテゴリーコード "
'wSQL2 = wSQL2 & "   AND e.中カテゴリーコード = d.中カテゴリーコード"
'wSQL2 = wSQL2 & "   AND f.メーカーコード = a.メーカーコード"
'wSQL2 = wSQL2 & "   AND f.商品コード = a.商品コード "
'wSQL2 = wSQL2 & "   AND f.商品コード = a.商品コード "        '2005/07/19
'wSQL2 = wSQL2 & "   AND h.大カテゴリーコード = e.大カテゴリーコード"   '2010/06/08 an add
'
'if CategoryCd <> "" then
'    wSQL2 = wSQL2 & "  AND f.カテゴリーコード = '" & CategoryCd & "'"    '2005/07/19
'end if
'
...
'end if
'
''---- ORDER BY  2010/03/18 an changed
'v_order = ""
'
'Select Case i_sort
'    Case "Price_ASC"
'    	v_order = " ORDER BY 実販売単価, c.メーカー名, a.商品名"
'    Case "Price_DESC"
'    	v_order = " ORDER BY 実販売単価 DESC, c.メーカー名, a.商品名"
'    Case "MakerName_ASC"
'    	v_order = " ORDER BY c.メーカー名, a.商品名"
'    Case "ProductName_ASC"
'    	v_order = " ORDER BY a.商品名"
'    Case "NewArrivals"
'    	v_order = " ORDER BY a.発売日 DESC, c.メーカー名, a.商品名"
'    Case "Reviews"
'    	v_order = " ORDER BY レビュー評価平均 DESC, c.メーカー名, a.商品名"
'    Case "ReviewCount"
'        v_order = " ORDER BY レビュー数 DESC, c.メーカー名, a.商品名"        '2010/07/12 st add
'    Case "Zaiko_DESC"
'    	v_order = " ORDER BY 完売区分, 在庫有無 DESC, c.メーカー名, a.商品名"
'    Case "PriceDown_ASC"
'        v_order = " ORDER BY 値下げ幅, c.メーカー名, a.商品名"
'    Case "PriceDown_DESC"
'        v_order = " ORDER BY 値下げ幅 DESC, c.メーカー名, a.商品名"
'    Case "Update_ASC"
'        v_order = " ORDER BY a.前回単価変更日, c.メーカー名, a.商品名"
'    Case Else
'    	v_order = " ORDER BY a.前回単価変更日 DESC, c.メーカー名, a.商品名"
'End Select
'
''---- 該当商品一覧SQL
...
'wSQL2 = wSQL2 & "        LEFT JOIN Web商品 b WITH (NOLOCK) "
'wSQL2 = wSQL2 & "          ON     b.メーカーコード = a.メーカーコード "
'wSQL2 = wSQL2 & "             AND b.商品コード     = a.商品コード "

'--- WHERE
wSQL2 = wSQL2 & "WHERE "
wSQL2 = wSQL2 & "        a.セール区分番号 = 4 "						' プライスダウン (PriceDownSale) セール区分番号 : 4
wSQL2 = wSQL2 & "    AND a.前回単価変更日 >= DATEADD(d, " & CLng(i_LastUpdate) * -1  & ", GETDATE()) "		' 前回価格更新日の条件
...
Select Case i_sort
    Case "Price_ASC"
        v_order = " ORDER BY a.実販売単価, a.メーカー名, a.商品名 "
    Case "Price_DESC"
        v_order = " ORDER BY a.実販売単価 DESC, a.メーカー名, a.商品名 "
    Case "MakerName_ASC"
    ...
    Case "Zaiko_DESC"
        v_order = " ORDER BY a.完売区分, 在庫有無 DESC, a.メーカー名, a.商品名 "
    Case "PriceDown_ASC"
        v_order = " ORDER BY 値下げ幅, a.メーカー名, a.商品名 "
    Case "PriceDown_DESC"
        v_order = " ORDER BY 値下げ幅 DESC, a.メーカー名, a.商品名 "
    Case "Update_ASC"
        v_order = " ORDER BY a.前回単価変更日, a.メーカー名, a.商品名 "
    Case Else
        v_order = " ORDER BY a.前回単価変更日 DESC, a.メーカー名, a.商品名 "
End Select
...
if RSv("メーカーロゴファイル名") <> "" then
    wHTML = wHTML & "<img src='maker_img/" & RSv("メーカーロゴファイル名") & "' alt='" & RSv("メーカー名") & "'>" & vbNewLine
end if
...
<title>プライスダウン情報 <%=wTitle%> 一覧｜サウンドハウス</title>
<meta name="Description" content="人気の商品がさらにお得に。お買い得情報をお見逃しなく！">
<meta name="keywords" content="得割市場,アウトレット,数量限定バーゲン,プライスダウン情報">
...
//    前回単価更新日 onChange
//
function LastUpdate_onChange(){
    for (var i=0; i<document.f_LastUpdate.LastUpdate.options.length; i++){
        if (document.f_LastUpdate.LastUpdate.options[i].selected == true){
            document.f_search.i_LastUpdate.value = document.f_LastUpdate.LastUpdate.options[i].value;
            break;
        }
    }
    for (var i=0; i<document.f_LastUpdate.sort.options.length; i++){
        if (document.f_LastUpdate.sort.options[i].selected == true){
            document.f_search.i_sort.value = document.f_LastUpdate.sort.options[i].value;
            break;
        }
    }
    document.f_search.i_page.value = "1";
    document.f_search.submit();
}

//   該当商品ありの場合のみ実行
function preset_values2(){

//    Page Size
    for (var i=0; i<document.f_page_size.page_size.options.length; i++){
        if (document.f_page_size.page_size.options[i].value == document.f_search.i_page_size.value){
            document.f_page_size.page_size.options[i].selected = true;
            break;
        }
    }


//   Sort（共通の並び替え） 2010/03/18 an add
    for (var i=0; i<document.f_page_size.sort.options.length; i++){
        if (document.f_page_size.sort.options[i].value == document.f_search.i_sort.value){
            document.f_page_size.sort.options[i].selected = true;
            break;
        }
    }
}
...
<%=wFootprintHTML%>
        ...
        <div class="tab" id="pricedownsale">
            <ul>
                ...
                <li><a href="BargainSale.asp"><img src="images/tab_specialpricesale.png" alt="衝撃特価品"></a></li>
            </ul>
        </div>
        <div id="info_box">
          <form id="form1" name="f_LastUpdate">
          <select id="LastUpdate" name="LastUpdate" onChange="LastUpdate_onChange();">
          ...
          <span class="style1">日前のプライスダウン情報</span>
          <select id="sort" name="sort" onChange="LastUpdate_onChange();">
            <option value="">更新順▼</option>
            <option value="Update_ASC">更新順▲</option>
            <option value="PriceDown_DESC">値下げ幅順▼</option>
            <option value="PriceDown_ASC">値下げ幅順▲</option>
          </select>
          </form>
        </div>
...
<!-- 絞込検索用Form START -->
      <form name="f_search" method="get" action="PriceDownSale.asp">
        ...
        <input type="hidden" name="i_LastUpdate" value="<%=i_LastUpdate%>">
      </form>
<!-- 絞込検索用Form END -->
<%=wNoDataHTML%>
      <!--/#contents --></div>
...
<%
'----NAVI用パラメータセット
'NAVILargeCategoryCd = LargeCategoryCd
'NAVIMidCategoryCd = MidCategoryCd
NAVISearchListMakerListHTML = wNaviMakerHTML
NAVISearchListLargeCategoryListHTML = wNaviLargeCategoryHTML   '2010/06/08 an add
NAVISearchListMidCategoryListHTML = wNaviMidCategoryHTML       '2010/06/08 an add
...
for (var i=0; i<document.f_LastUpdate.LastUpdate.options.length; i++){
    if (document.f_LastUpdate.LastUpdate.options[i].value == document.f_search.i_LastUpdate.value){
        document.f_LastUpdate.LastUpdate.options[i].selected = true;
        break;
    }
}
//   Sort（プライスダウン独自の並び替え）
for (var i=0; i<document.f_LastUpdate.sort.options.length; i++){
    if (document.f_LastUpdate.sort.options[i].value == document.f_search.i_sort.value){
        document.f_LastUpdate.sort.options[i].selected = true;
        break;
    }
}
