...
'
'更新履歴
'2010/06/7 ta 新規作成(RankingReviewPoint.aspを参照)
'2011/08/01 an #1087 Error.aspログ出力対応
'2012/01/20 GV データ取得 SELECT文へ LACクエリー案を適用
'2012/01/23 GV 「商品レビュー」テーブルから「商品レビュー集計」テーブル使用に変更 (CreateRankingReviewHTML()プロシージャ)
...
Do Until RS.EOF = true
	...
	vCount = vCount + 1
	wHTML = wHTML & "<a href='RankingReview.asp?LargeCategoryCd=" & RS("大カテゴリーコード") & "'>" & RS("大カテゴリー名") & "</a>"

	if RS("大カテゴリーコード") = LargeCategoryCd then
		wLargeCategoryName = RS("大カテゴリー名")  'レビュー数一覧のタイトルで使用
	end if
...
wSQL = wSQL & "SELECT DISTINCT TOP 25 "
wSQL = wSQL & "      a.レビュー件数 AS レビューコメント数 "
wSQL = wSQL & "    , b.メーカーコード "
wSQL = wSQL & "    , b.カテゴリーコード "
wSQL = wSQL & "    , b.商品コード "
wSQL = wSQL & "    , b.商品名 "
...
wHTML = wHTML & "      <div id='th_count'>レビュー数</div>" & vbNewLine
...
'2014/03/19 GV mod end   <----
		...
		wHTML = wHTML & "          <strong>" & RSv("レビューコメント数") & "</strong>" & vbNewLine
		wHTML = wHTML & "        </div>" & vbNewLine
...
<!--
  <div id="path_box"><div id="path_box_inner01"><div id="path_box_inner02">
    ...
    <ul id="path">
      <li class="now">レビュー数</li>
    </ul>
  </div></div></div>
  <h1 class="title">レビュー数</h1>
-->
